...
public class FabricanteDAOJDBC implements FabricanteDAO {
    ...
    public int inserir(Fabricante fabricante) throws ClassNotFoundException, SQLException, SQLIntegrityConstraintViolationException {
        ...
        sqlBuilder
                .append("insert into fabricante(fabricante, data) ")
                .append("VALUES (?, ?)");
     
        String insert = sqlBuilder.toString();
        int linha = 0;
        try {  
            linha = DAOGenerico.executarComando(insert, fabricante.getFabricante(),
                                                        fabricante.getData());
    ...
    public List<Fabricante> listar() {
        ...
        try {        
            ...
            while (rset.next()) {
                ...
                fabricante.setFabricante(rset.getString("fabricante"));
                fabricante.setData(rset.getDate("data"));
    ...
    public int editar(Fabricante fabricante) {
      StringBuilder sqlBuilder = new StringBuilder();
        ...
        int linha = 0;
        try {
            linha = DAOGenerico.executarComando(update, fabricante.getFabricante(),
                                                        fabricante.getId());
        } catch (Exception e) { 
            e.printStackTrace();
        } 
